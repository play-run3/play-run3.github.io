...
async function checkUrl(rawUrl, options = {}) {
  ...
  const method = String(options.method || 'GET').toUpperCase() === 'HEAD' ? 'HEAD' : 'GET';
...
app.post('/api/check', rateLimit, async (req, res) => {
  ...
  // Do NOT use req.on('close') here. In Node/Express, the request can emit
  // 'close' after the request body has been received, which would make the
  // workers exit before checking even the first URL.
